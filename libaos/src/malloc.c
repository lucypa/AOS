...
char sos_morecore_area[MORECORE_AREA_BYTE_SIZE];

/* Pointer to free space in the morecore area. */
static uintptr_t morecore_base = (uintptr_t) &sos_morecore_area;
static uintptr_t morecore_top = (uintptr_t) &sos_morecore_area[MORECORE_AREA_BYTE_SIZE];

/* Actual morecore implementation
   returns 0 if failure, returns newbrk if success.
*/
static long sos_brk(uintptr_t newbrk)
{
    uintptr_t ret;
    ...
    if (!newbrk) {
        ret = morecore_base;
    } else if (newbrk < morecore_top && newbrk > (uintptr_t)&sos_morecore_area[0]) {
...
static long sos_sbrk(uint32_t increment)
{
	uintptr_t ret = morecore_base;

	if (!sos_brk(ret + increment)) {
		ret = (void *)-1;
	}
...
/* Request that the OS increment the program's data space.  sbrk changes the
   ...
   */
  freemem = sos_sbrk(nunits * sizeof(Header));
